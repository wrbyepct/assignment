...
#### 為什麼選擇下載 CSV 而非使用 API？

在設計 ETL 之前，我考慮了兩種取得資料的方式：

| 方式 | 優點 | 缺點 | 適用場景 |
|------|------|------|----------|
| **CSV 下載** | 一次取得完整資料、離線可處理、實作簡單 | 無法即時更新、檔案較大 | ✅ 本專案：每日批次全量更新 |
| **API 分頁** | 可即時查詢、支援條件篩選、支援增量同步 | 需多次請求、可能受 rate limit 限制、實作較複雜 | 即時查詢、增量同步場景 |
